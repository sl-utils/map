import type { AMapMapsEvent as _MapAEvent, MapBounds, MapCanvasLayer, MapMouseEvent, MapPosition, MapSize, MOpt } from "../map";
import { Layer as _MapLLayer, LayerOptions as _MapLLayerOptions, LeafletMouseEvent as _MapLEvent, Map as _MapL } from "leaflet";
import type { CustomLayerInterface as _MapMLayer, MapMouseEvent as _MapMEvent, StyleSpecification as _MapMStyle } from 'maplibre-gl';
import { Map as _MapM } from 'maplibre-gl';
...
* @returns 两点与正北方的角度
*/
declare function getAngle(map: MapType, lnglatA: [number, number], lnglatB: [number, number]): number;
/**
 * 获取地图边界
 * @params map 地图实例
 * @returns 地图边界
 *  */
declare function getBounds(map: MapType): MapBounds;
 ...
 * @returns 两点间的距离(米)
 */
declare function getDistance(lnglatA: [number, number], lnglatB: [number, number], map: MapType): number;
/** 将坐标系转换为经纬度数
 * @param map 地图实例
 * @param point 像素点位
 * @returns lnglat [lng,lat]
 */
declare function getLngLatByPoint(map: MapType, point: [number, number] | undefined): [number, number];
/** 获取指定间隔距离的经度差值
 * @param map 地图实例
 * @param 间隔距离 @default 100
 * @param 纬度点位集合(纬度不同，相同距离经度变化差值不一样)
 */
declare function getLngDiffByDistance(map: MapType, distance: number, lnglats: [number, number][]): number;
/** 得到坐标系点位
 * @param map 当前的地图
 * @param lnglat [经度,纬度]
 * @returns lnglat有效时返回 [x,y] , 无效时返回 [-1000, -1000]
 */
declare function getPointByLnglat(map: MapType, lnglat: [number, number] | undefined): [number, number];
/** 将经纬度数组转换为坐标系
 * @param map 当前的地图
 * @param lnglats [经度,纬度][]
 * @returns lnglats有效时返回 [x,y][]
 */
declare function getPointsByLnglats(map: MapType, lnglats: [number, number][] | undefined): [number, number][];
/**
 * 经纬度数组 转 屏幕像素坐标
 * @param map 当前的地图
 * @param lng 经度
 ...
 * @param zoom 缩放级别
 */
declare function getProjectedPointByLnglat(map: MapType, lng: number, lat: number, zoom: number): [number, number];
/** 将经纬度数组转换为坐标系
 * @param map 当前的地图
 * @param lnglats [经度,纬度][]
 * @param zoom 缩放级别
 ...
 */
declare function getProjectedPointByLnglats(map: MapType, lnglats: [number, number][] | undefined, zoom: number): [number, number][];
/**对大小进行解析设置
 * @param map  当前的地图
 * @param info 大小信息和位置信息
 * @returns [x轴的像素大小 number,y轴的像素大小 number]
 */
declare function getSizeByMap(map: MapType, info: MapPosition & MapSize): [number, number];
/**获取地图实例的大小宽高
 * @param map 地图实例
 * @returns { w: number, h: number }
 */
declare function getMapSize(map: MapType): {
    w: number;
    h: number;
};
/**
...
*/
declare function getMapMouseEvent(e: MapEventType, map: MapType): MapMouseEvent;
/**设置地图状态
...
*/
declare function setMapStatus(map: MapType, key: 'dragEnable', flag: boolean): void;
/**
 * 设置地图中心
 * @param map 地图实例
 ...
 */
declare function setViewCenter(map: MapType, center: [number, number], zoom: number, offset?: [number, number]): void;
/**
 * 设置地图最合适缩放位置中心
 * @param map 地图实例
 * @param allPoints 大于等于2个点
 */
declare function setFitBounds(map: MapType, allPoints: [number, number][]): void;
/**
 * 设置地图最合适缩放位置中心
 * @param map 地图实例
 ...
 */
declare function setFitBounds(map: MapType, southwest: [number, number], northeast: [number, number]): void;
/**将数值转换为经纬度字符串
 * @param value 数值
 * @param ifLng 是否是经度
 ...
 * @returns 经纬度[lng, lat]
 */
declare function getLngLatByEvent(e: MapEventType): [number, number] | null;
/**创建地图图层
 * @param map 地图实例
 * @param layer 图层
 * @returns 图层
*/
declare function createMapLayer(map: _MapM, that: MapCanvasLayer): _MapMLayer;
declare function createMapLayer(map: _MapL, that: MapCanvasLayer): _MapLLayer;
declare function createMapLayer(map: _MapA, that: MapCanvasLayer): _MapALayer;
/**根据类型获取地图实例
 * @param mapType 地图类型
 * @returns 地图实例类型
*/
declare function getMapInstance(type: 'L' | 'A' | 'B' | 'M', ele: string, opt: Partial<MOpt>): Promise<MapType>;
