...
export class MapCanvasFixedHeat {
  ...
  /** 100m网格聚合后的数据 */
  private aggregatedData: [number, number, number][] = [];
  /** 强度最大值（用于归一化） */
  private maxIntensity: number = 1;
  /** 当前渲染缩放比例（大区域自动缩小） */
  ...
  /** 热力图对应的经纬度边界 */
  private bounds: { minLng: number; maxLng: number; minLat: number; maxLat: number; } | null = null;
   ...
   * @returns [经度, 纬度, 平均强度]
   */
  private buildHeatPoints(data: [number, number, number][]): [number, number, number][] {
    if (!data.length) return [];
    const GRID = 0.001;
    const grid = new Map<string, { sumIntensity: number; count: number }>();
    const getKey = (lat: number, lng: number) => {
    ...
    data.forEach(([lng, lat, intensity]) => {
      ...
      if (!grid.has(key)) grid.set(key, { sumIntensity: 0, count: 0 });
  ...
  private renderToImage(): void {
    ...
    /**5.转为本地画布坐标 */
    const localPoints: [number, number, number][] = projPoints.map(p => {
