...
export class MapPluginHeat extends MapCanvasLayer {
    ...
    genShadowRadius(r, blur = 15) {
        let circle = this._circleShadow = SLUCanvas.createCanvas(), ctx = circle.getContext('2d'), r2 = this._r = r + blur;
    ...
    genGradient(grad) {
        let canvas = this._gradEl = SLUCanvas.createCanvas(), ctx = canvas.getContext('2d'), gradient = ctx.createLinearGradient(0, 0, 0, 256);
        canvas.width = 1;
        canvas.height = 256;
        for (let i in grad) {
