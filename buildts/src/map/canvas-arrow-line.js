...
exports.MapCanvasArrowLine = void 0;
const slu_map_1 = require("../utils/slu-map");
const slu_canvas_1 = require("../canvas/slu-canvas");
const slu_canvas_img_1 = require("../canvas/slu-canvas-img");
const ARROW_URL = "/assets/images/direction-arrow.png";
class MapCanvasArrowLine {
    ...
    initResource() {
        slu_canvas_img_1.SLUCanvasImg.loadImg([this.imgUrl]);
    }
    setAllLines(lines) {
        this.allLines = lines;
        this.update();
    ...
    update() {
        this.allPoints = this.allLines.map((line) => {
            ...
            return (0, slu_map_1.u_mapGetPointsByLatlngs)(this.map, latlngs);
        });
        this.draw();
    }
        ...
        for (let i = 0, len = this.allPoints.length; i < len; i++) {
            ...
            if (isBezier) {
                for (let j = 1, len2 = this.allPoints[i].length; j < len2; j++) {
                    ...
                    const ctrl = slu_canvas_1.SLUCanvas.getBezierCtrlPoint(prev, cur, degree);
                    const maxSlice = 50;
    ...
    createPattern() {
        ...
        const img = slu_canvas_img_1.SLUCanvasImg.ImageCache[this.imgUrl];
        if (!img)
            return null;
...
exports.MapCanvasArrowLine = MapCanvasArrowLine;
