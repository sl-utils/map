...
exports.MapPluginPlot = void 0;
const map_1 = require("../map");
const plugin_draw_1 = require("./plugin-draw");
const slu_map_1 = require("../utils/slu-map");
const slu_array_1 = require("../utils/slu-array");
class MapPluginPlot extends map_1.MapCanvasLayer {
    constructor(sluMap, options) {
        ...
        this.eventClick = (e) => {
            this.eventClickTimer = setTimeout(() => {
                ...
                if (!plot)
                    return;
                let { latlng } = (0, slu_map_1.u_mapGetMapMouseEvent)(e, this.map);
                let lat84 = latlng.lat, lng84 = latlng.lng;
                if ((0, slu_map_1.u_tsMapisAmap)(this.map)) {
                    const { lat, lng } = (0, slu_map_1.u_mapTogcj02gps84)(latlng.lng, latlng.lat);
                    lat84 = lat, lng84 = lng;
                }
    ...
    drawPlot(layer, plotInfo, type) {
        ...
        switch (info.type) {
            ...
            case 'circle':
                ...
                if (!elatlng) {
                    let [lat, lng] = slatlng;
                    let lngDis = (0, slu_map_1.u_mapGetLngDiffByDistance)(this.map, rail, [[lat, lng]]);
    ...
    calcRadius(latLngs) {
        if (latLngs.length < 2)
            return 0;
        let [px1, py1] = (0, slu_map_1.u_mapGetPointByLatlng)(this.map, latLngs[0]);
        let [px2, py2] = (0, slu_map_1.u_mapGetPointByLatlng)(this.map, latLngs[1]);
    ...
    setPolygonEditPoint(plotInfo) {
        ...
        for (let i = 0, len = latLngs.length; i < len; i++) {
            ...
            if (!this.curPoint) {
                ...
                let [px1, py1] = (0, slu_map_1.u_mapGetPointByLatlng)(this.map, s);
                let [px2, py2] = (0, slu_map_1.u_mapGetPointByLatlng)(this.map, e);
                let x = (px1 + px2) / 2, y = (py1 + py2) / 2;
                let point = (0, slu_map_1.u_mapGetLatLngByPoint)(this.map, [x, y]);
    ...
    addEvent(latLng, i, plotInfo, eves, ifVirtual) {
        ...
        eves.push({
            ...
            type: 'mousedown',
            cb: () => {
                (0, slu_map_1.u_mapSetMapStatus)(map, 'dragEnable', false);
                if (ifVirtual) {
                    (0, slu_array_1.u_arrAddItemsIndex)(latLngs, [latLng], i);
                    this.cbPointChange && this.cbPointChange(this.plotAni);
                }
                this._redraw();
                let moveCb = (e) => {
                    let { latlng: eLatlng } = (0, slu_map_1.u_mapGetMapMouseEvent)(e, this.map);
                    let lat84 = eLatlng.lat, lng84 = eLatlng.lng;
                    if ((0, slu_map_1.u_tsMapisAmap)(this.map)) {
                        const { lat, lng } = (0, slu_map_1.u_mapTogcj02gps84)(eLatlng.lng, eLatlng.lat);
                        lat84 = lat, lng84 = lng;
                    }
                ...
                let upCb = () => {
                    ...
                    (0, slu_map_1.u_mapSetMapStatus)(map, 'dragEnable', true);
                    this.cbPointChange && this.cbPointChange(this.plotAni);
                    this._redraw();
                };
...
exports.MapPluginPlot = MapPluginPlot;
