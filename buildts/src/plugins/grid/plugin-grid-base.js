...
exports.MapPluginGridBase = void 0;
const map_1 = require("../../map");
const canvas_1 = require("../../canvas");
const slu_map_1 = require("../../utils/slu-map");
const slu_worker_1 = require("../../utils/slu-worker");
class MapPluginGridBase extends map_1.MapCanvasLayer {
    constructor(map, options) {
        ...
        this.dataLength = 1;
        this.worker = new slu_worker_1.SLUWorker('grid', (data) => this.workerCb(data));
        this.workerId = 0;
    ...
    interpolateFieldByWorker(bounds) {
        let [lat, lng] = (0, slu_map_1.u_mapGetLatLngByPoint)(this.map, [0, 0]);
        let [, lng1] = (0, slu_map_1.u_mapGetLatLngByPoint)(this.map, [1, bounds.height]);
        let lats = [];
        for (let i = 0, len = bounds.height; i <= len; i++)
            lats[i] = (0, slu_map_1.u_mapGetLatLngByPoint)(this.map, [0, i])[0];
        this.worker.post({
            id: this.workerId++,
            width: bounds.width,
    ...
    genShadowRadius(r, blur = 15) {
        let circle = canvas_1.SLUCanvas.createCanvas(), ctx = circle.getContext('2d'), r2 = r + blur;
    ...
    genGradient(grad) {
        let canvas = canvas_1.SLUCanvas.createCanvas(), ctx = canvas.getContext('2d'), gradient = ctx.createLinearGradient(0, 0, 0, 256);
        canvas.width = 1;
        canvas.height = 256;
        for (const i in grad)
...
exports.MapPluginGridBase = MapPluginGridBase;
