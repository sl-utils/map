...
exports.PluginCoastlineMask = void 0;
const bbox_clip_1 = __importDefault(require("@turf/bbox-clip"));
const slu_map_1 = require("../utils/slu-map");
const clipGeometryTypes = ['LineString', 'MultiLineString', 'Polygon', 'MultiPolygon'];
class PluginCoastlineMask {
    ...
    clipGeoJSON(geojson, bbox) {
        ...
        for (let i = 0, len = features.length; i < len; i++) {
            ...
            for (let j = 0, len2 = bboxes.length; j < len2; j++) {
                try {
                    const clipped = (0, bbox_clip_1.default)(feature, bboxes[j]);
        ...
        for (let r = 0, len = coordinates.length; r < len; r++) {
            ...
            for (let o = 0, len2 = offsets.length; o < len2; o++) {
                ...
                for (let i = 0, len3 = ring.length; i < len3; i++) {
                    ...
                    prevLng = lng;
                    const [x, y] = (0, slu_map_1.u_mapGetPointByLatlng)(this.map, [lat, lng]);
                    if (first) {
                        ctx.moveTo(x, y);
                        first = false;
                    }
...
exports.PluginCoastlineMask = PluginCoastlineMask;
